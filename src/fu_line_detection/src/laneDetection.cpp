...
//  remove all //-------------------------------- added by kaustubh ------------------------------------  if you don't want to send micro secs servo value to /cam_yaw topic

//#define PAINT_OUTPUT
#define PUBLISH_DEBUG_OUTPUT
//#define THRESHOLD_RED_ON
#define THRESHOLD_RED_OFF

//#define TOP_HALF_IMAGE
//#define BOT_HALF_IMAGE
#define FULL_IMAGE
...
image_transport::CameraPublisher image_publisher_edges;
image_transport::CameraPublisher image_publisher_lane_edges;
image_transport::CameraPublisher image_publisher_ransac;
image_transport::CameraPublisher image_publisher_lane_markings;
image_transport::CameraPublisher image_publisher_ipmap;
    ...
    : nh_(nh), priv_nh_("~")
{
    ...
    priv_nh_.param<std::string>(node_name+"/camera_name", camera_name, "/csi_cam/image_raw"); 
    ...
    priv_nh_.param<int>(node_name+"/roi_top_w", roi_top_w, 9999);
    priv_nh_.param<int>(node_name+"/roi_bottom_w", roi_bottom_w, 9999);
    
    priv_nh_.param<int>(node_name+"/maxYRoi", maxYRoi, 9999);
    priv_nh_.param<int>(node_name+"/minYDefaultRoi", minYDefaultRoi, 9999);
    priv_nh_.param<int>(node_name+"/minYPolyRoi", minYPolyRoi, 9999);
    ...
    #ifdef TOP_HALF_IMAGE
        ipMapper = IPMapper(cam_w, cam_h/2, f_u, f_v, c_u, c_v, cam_deg, cam_height);
    #endif

    #ifdef BOT_HALF_IMAGE
        ipMapper = IPMapper(cam_w, cam_h/2, f_u, f_v, c_u, c_v, cam_deg, cam_height);
    #endif
    ...
    //read_images_ = nh.subscribe("/csi_cam/image_raw", MY_ROS_QUEUE_SIZE, &cLaneDetectionFu::ProcessInput,this);
    ...
    cam_yaw_pub = nh.advertise<cam_data::Yaw>("/cam_yaw", MY_ROS_QUEUE_SIZE); //-------------------------------- added by kaustubh ------------------------------------
    ...
    image_publisher = image_transport.advertiseCamera("/lane_model/lane_model_image", MY_ROS_QUEUE_SIZE);
        ...
        image_publisher_ipmap = image_transport.advertiseCamera("/lane_model/ipmapped", MY_ROS_QUEUE_SIZE);
        image_publisher_edges = image_transport.advertiseCamera("/lane_model/all_edges", MY_ROS_QUEUE_SIZE);
        image_publisher_lane_edges = image_transport.advertiseCamera("/lane_model/lane_edges", MY_ROS_QUEUE_SIZE);
...
void cLaneDetectionFu::ProcessInput(const sensor_msgs::Image::ConstPtr& msg)
{
    ...
    #ifdef THRESHOLD_RED_OFF
        cv_ptr = cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::MONO8); //MONO8==> graysacle 
        cv::Mat image = cv_ptr->image.clone(); 
        cv::bitwise_not ( image, image ); //invert graysacle (can make this cv::cuda::)
        cv::Mat thresh_img = image.clone();
    #endif

    #ifdef THRESHOLD_RED_ON //---gpu changed all above to work on cuda
        cv_ptr = cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::BGR8); //BGR8==> colour
        cv::Mat image = cv_ptr->image.clone();
	cv::cuda::GpuMat gpu_image(image);
        cuda::cvtColor(gpu_image, gpu_image, CV_BGR2HSV); // convert from bgr to hsv
        //cv::inRange(gpu_image, cv::Scalar(160, 100, 100), cv::Scalar(179, 255, 255), gpu_image); // filter pre_image for red in hsv range and copy to image
	// no cuda::inRange exists so 

	// init some matrices
	cuda::GpuMat shsv[3];
	cuda::GpuMat thresc[3];
	cuda::GpuMat temp;
	cuda::GpuMat thresh_gpu_img;
	// Split HSV 3 channels
	cuda::split(gpu_image, shsv);
	// Threshold HSV channels
	cuda::threshold(shsv[0], thresc[0], 160, 179, THRESH_BINARY);
	cuda::threshold(shsv[1], thresc[1], 100, 255, THRESH_BINARY);
	cuda::threshold(shsv[2], thresc[2], 100, 255, THRESH_BINARY);
	// Bitwise AND the channels
	cuda::bitwise_and(thresc[0], thresc[1],temp);
	cuda::bitwise_and(temp, thresc[2], thresh_gpu_img);
        cv::Mat thresh_img(thresh_gpu_img); // had to convert from gpuimg to img because of ipmapper
    #endif

    #ifdef FULL_IMAGE
        cv::Mat cut_image = thresh_img(cv::Rect(0,0,cam_w,cam_h)); 
    #endif

    #ifdef TOP_HALF_IMAGE
        cv::Mat cut_image = thresh_img(cv::Rect(0,0,cam_w,cam_h/2)); 
    #endif

    #ifdef BOT_HALF_IMAGE
        cv::Mat cut_image = thresh_img(cv::Rect(0,cam_h/2,cam_w,cam_h/2)); 
    #endif

    cv::Mat remapped_image = ipMapper.remap(cut_image);

    // alternate IP Mapping
    //cv::cuda::GpuMat remapped_gpu_image;
    //cv::Mat mapping_mat33 = ipMapper.remap(); //instead of remapping in ipMapper, only get mappingMatrix
    //cuda::warpPerspective(thresh_gpu_img, remapped_gpu_image, mapping_mat33, image.size(), INTER_LINEAR, BORDER_CONSTANT);
    //cv::Mat remapped_image(remapped_gpu_image); //convert gpu_img back to img for use below
    ...
    #ifdef PUBLISH_DEBUG_OUTPUT
	cv::Mat tempImage = remapped_image.clone();
        cv::cvtColor(tempImage,tempImage,CV_GRAY2BGR);
        pubRGBImageMsg(tempImage, image_publisher_ipmap);
    #endif
    ...
    pubRGBImageMsg(transformedImagePaintable, image_publisher_edges);
    ...
    pubRGBImageMsg(transformedImagePaintable, image_publisher_lane_edges);
    ...
    //---------------------- DEBUG OUTPUT GROUPED LANE MARKINGS ---------------------------------// grouped lane markings has the green ROI box
    #ifdef PUBLISH_DEBUG_OUTPUT
        transformedImagePaintable = transformedImage.clone();
        cv::cvtColor(transformedImagePaintable,transformedImagePaintable,CV_GRAY2BGR);
        ...
        cv::line(transformedImagePaintable,p1l,p2l,cv::Scalar(0,0,255)); //RED LINE (LEFTMOST LANE MARKER SEPARATOR)

        cv::Point2d p1c(defaultXCenter,minYPolyRoi);
        cv::Point2d p2c(defaultXCenter,maxYRoi-1);
        cv::line(transformedImagePaintable,p1c,p2c,cv::Scalar(0,255,0)); //GREEN LINE (MIDDLE)

        cv::Point2d p1r(defaultXRight,minYPolyRoi);
        cv::Point2d p2r(defaultXRight,maxYRoi-1);
        cv::line(transformedImagePaintable,p1r,p2r,cv::Scalar(255,0,0)); //BLUE LINE

        cv::Point2d p1(proj_image_w_half-(roi_bottom_w/2),maxYRoi-1);
        cv::Point2d p2(proj_image_w_half+(roi_bottom_w/2),maxYRoi-1);
        cv::Point2d p3(proj_image_w_half+(roi_top_w/2),minYPolyRoi);
        cv::Point2d p4(proj_image_w_half-(roi_top_w/2),minYPolyRoi);
        cv::line(transformedImagePaintable,p1,p2,cv::Scalar(0,200,0)); //GRREN BOX SIDES
        cv::line(transformedImagePaintable,p2,p3,cv::Scalar(0,200,0));
        cv::line(transformedImagePaintable,p3,p4,cv::Scalar(0,200,0));
        cv::line(transformedImagePaintable,p4,p1,cv::Scalar(0,200,0));
...
//--------------------------------------------------------------------------------------------------------------------Below this line only fns (and main fn). Main function calls above class (imp!). 
...
void cLaneDetectionFu::pubAngle()
{
    ...
    cam_data::Yaw cam_yaw_msg;  //-------------------------------- added by kaustubh ------------------------------------

    if (!lanePolynomial.hasDetected()) {
	/*angleMsg.data = -1;
	publish_angle.publish(angleMsg);
        cam_yaw_msg.yaw_num = -1;  //-------------------------------- added by kaustubh ------------------------------------
        cam_yaw_pub.publish(cam_yaw_msg);  //-------------------------------- added by kaustubh ------------------------------------
	*/	
	return;
    }
    ...
    cam_yaw_msg.yaw_num = result*(600/60) + 1600;  //-------------------------------- added by kaustubh ------------------------------------
    cam_yaw_pub.publish(cam_yaw_msg);  //-------------------------------- added by kaustubh ------------------------------------
}
